...
#ifndef RX_RECEIVE_STACK_SIZE
#define RX_RECEIVE_STACK_SIZE      configMINIMAL_STACK_SIZE
#endif

#ifndef RX_RECEIVE_TASK_PRIO
#define RX_RECEIVE_TASK_PRIO            20
#endif
...
static void rx_receive_task(void *arg);
...
void rx_receive_init(void)
{
  ...
  static StackType_t  xStack[RX_RECEIVE_STACK_SIZE];

  // Create Blink Task without using any dynamic memory allocation
  xHandle = xTaskCreateStatic(rx_receive_task,
                              "UART Receive",
                              RX_RECEIVE_STACK_SIZE,
                              ( void * ) NULL,
                              tskIDLE_PRIORITY + 1,
                              xStack,
                              &xTaskBuffer);
  ...
  xReturned = xTaskCreate(Rx_Receive_task,
                          "UART Receive",
                          RX_RECEIVE_TASK_STACK_SIZE,
                          ( void * ) NULL,
                          tskIDLE_PRIORITY + 1,
                          &xHandle);
